...
namespace iproby.Controllers
{
    public class CatalogController : Controller
    {
        ...
        public ActionResult Rabota(int type_id=111, string target = "clients")
        {
            if (type_id == 111) {

                return Announs(target = "clients");
            }
            else { 
            var announ_id_arr = (from b in db.announs
                                 where b.type_id == type_id
        ...
        public ActionResult Service(int type_id = 111, string target = "workers")
        {
            if (type_id == 111)
            {
                return Announs(target = "workers");
            }
            else
            {
                var announ_id_arr = (from b in db.announs
